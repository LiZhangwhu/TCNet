...
def rand_flip2(img_s, label_s, label_mask_s, img, label):
    ...
    if r < 0.25:
        ...
        return np.flip(img_s, axis=0).copy(), np.flip(label_s, axis=0).copy(), np.flip(label_mask_s, axis=0).copy(), np.flip(img, axis=0).copy(), np.flip(label, axis=0).copy()
    elif r < 0.75:
        return np.flip(img_s, axis=1).copy(), np.flip(label_s, axis=1).copy(), np.flip(label_mask_s, axis=1).copy(), np.flip(img, axis=1).copy(), np.flip(label, axis=1).copy()
    else:
        return img_s[::-1, ::-1, :].copy(), label_s[::-1, ::-1].copy(), label_mask_s[::-1, ::-1].copy(), img[::-1, ::-1, :].copy(), label[::-1, ::-1].copy()
...
def find_all_mask(label,b):
    mask_onehot = mask_to_onehot(label,7)
