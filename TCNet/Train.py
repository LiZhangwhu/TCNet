...
def train(train_loader, net, criterion, optimizer, val_loader):
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            imgs_s, labels_s, label_mask_s, bin_label_s, imgs, labels = data
            if args['gpu']:
                imgs = imgs.cuda().float()
                labels = labels.cuda().long()
                imgs_s = imgs_s.cuda().float()
                labels_s = labels_s.cuda().long()
                bin_label_s = bin_label_s.cuda().long()
                label_mask_s = label_mask_s.cuda().long()

            optimizer.zero_grad()
            outputs, aux, aux2, score = net(imgs_s, imgs)

            alpha = calc_alpha(running_iter, all_iters)
            main_loss = criterion(outputs, labels)
            aux_loss = criterion(aux, labels_s)
            aux2_loss = criterion(aux2, label_mask_s)
            score_loss = criterion(score, bin_label_s)
            loss = main_loss + aux_loss + score_loss
            loss.backward()
            optimizer.step()
...
def validate(val_loader, net, criterion, curr_epoch):
    ...
    for vi, data in enumerate(val_loader):
        imgs_s, labels_s,_,_, imgs, labels = data
